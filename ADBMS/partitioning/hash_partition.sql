CREATE TABLE account_info_hash_partitioned (
    ID INT,
    Name VARCHAR(50),
    Age INT,
    Email VARCHAR(100)
);
INSERT INTO account_info_hash_partitioned (ID, Name, Age, Email) VALUES
...
ALTER TABLE account_info_hash_partitioned PARTITION BY HASH(ID) PARTITIONS 4;
SELECT * FROM account_info_hash_partitioned PARTITION(p3);